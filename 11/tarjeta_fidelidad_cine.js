...
function shouldBuyFidelity(times) {
  ...
  const total_ticket_price = TICKET_PRICE * times;
  let total_fidelity_price = FIDELITY_CARD_PRICE;
  for (let i = 1; i <= times; i++) {
    const discount_amount = TICKET_PRICE * Math.pow(DISCOUNT_PERCENTAGE, i);
    total_fidelity_price += discount_amount;
  }
  return total_fidelity_price < total_ticket_price;
}
