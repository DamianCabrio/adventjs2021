...
Se ha dado cuenta que en el taller nadie le presta atención a los nombres de los ficheros y a veces intentan guardar el mismo fichero más de una vez... 
así que es importante que gestionemos bien los nombres duplicados.

Tenemos que crear una función que al pasarnos un array de nombres de archivo devolvamos un array con el mismo número de elementos pero 
donde los nombres que se repetían se anexe al final (k) donde k sería el número de veces que se encontró repetido.
...
function fixFiles(files) {
  ...
  for (let i = 0; i < files.length; i++) {
    const file = files[i];
    const repeatedFileNumber = repeatedFiles[file];
    if (repeatedFileNumber) {
      files[i] = `${file}(${repeatedFileNumber})`;
      repeatedFiles[file] = repeatedFiles[file] + 1;
    } else {
      repeatedFiles[file] = 1;
    }
  }
  return files;
}
