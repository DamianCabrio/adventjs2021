...
y que sólo tenemos que asegurarnos que los números suben y bajan de forma correcta. También nos avisa que sólo pasaremos arrays de, como mínimo, tres posiciones.
...
function checkSledJump(heights) {
  ...
  for (let i = 0; i < heights.length; i++) {
    if (heights[i] > heights[i + 1] && canBeHigher) {
      canBeHigher = false;
    } else if (
      (heights[i] < heights[i + 1] && !canBeHigher) ||
