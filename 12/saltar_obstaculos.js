...
El problema es que nos ha dado la lista de posiciones de los obstáculos desordenada... 😅 aunque al menos nunca la posición 0 puede tener un obstáculo.
...
function getMinJump(obstacles) {
  ...
  let jumpNumber = 1;
  let jumpI = 0;
  while (jumpI <= sortedObstacles[sortedObstacles.length - 1]) {
    if (!sortedObstacles.find((obstacle) => obstacle === jumpI)) {
      jumpI = jumpI + jumpNumber;
    } else {
      jumpI = 0;
      jumpNumber++;
    }
